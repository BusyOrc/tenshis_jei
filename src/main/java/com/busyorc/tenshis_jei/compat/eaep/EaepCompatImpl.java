...
final class EaepCompatImpl {
        ...
        try {
            ...
            int moved = ServerBookmarkExternalStoragePull.pull(
                ...
                (target, amount) -> {
                    ...
                    if (extracted < amount) {
                        // 库存不足：对缺口触发 AE2 自动合成（异步，成品稍后入网后再拉一次）
    ...
    static void autoCraft(ServerPlayer player, List<com.busyorc.tenshis_jei.network.SnapshotDataPacket.Entry> entries) {
        try {
            WirelessTerminalLocator.LocatedTerminal located = WirelessTerminalLocator.find(player);
            if (located.isEmpty()) {
                return;
            ...
            for (com.busyorc.tenshis_jei.network.SnapshotDataPacket.Entry entry : entries) {
                if (entry.amount() <= 0) {
                    continue;
                }
     ...
     * 关键点：
     * 1) 没有样板的物品先 isCraftable 跳过（否则会触发无意义的异步计算）；
     * 2) 计算是异步的（后台 CRAFTING_POOL），绝不能在服务端主线程 job.get() 阻塞，
     *    改为后台线程等结果，再回到服务端主线程 submitJob。
     */
    private static void triggerAutoCraft(IGrid grid, Player player, AEItemKey what, long amount, IActionSource src) {
        try {
            ...
            // 关键：simRequester 必须提供网格节点，否则合成计算拿不到网络库存，
            // 会把一切都算成"缺失"而返回 simulation()==true（表现为"plan not craftable"）。
            ...
            Thread waiter = new Thread(() -> {
                try {
                    ICraftingPlan plan = job.get(30, java.util.concurrent.TimeUnit.SECONDS);
